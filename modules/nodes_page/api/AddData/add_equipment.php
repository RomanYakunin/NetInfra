...
try {
    ...
    if (isset($_POST['lldp_neighbors'])) {
        ...
        if (is_array($lldp)) {
            $pdo->prepare("DELETE FROM lldp_neighbors WHERE local_equipment_id = ?")->execute([$newId]);
            ...
            foreach ($lldp as $n) {
                ...
                $insLldp->execute([$newId, $localPort, $remoteHost, $remotePort ?: null, $remoteType, $remoteId]);
            }
        }
    }
