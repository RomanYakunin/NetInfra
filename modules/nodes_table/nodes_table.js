// modules/nodes_table/nodes_table.js – финальная версия, адаптированная под новую схему БД (group_id, equipment_groups)
...
function toggleNodeEquipment(row, nodeId) {
    ...
    fetch(`?ajax=get_equipment&node_id=${nodeId}&_=${Date.now()}`)
        .then(response => {
        ...
        .then(data => {
            ...
            if (groups.length === 0) {
                ...
                groups.forEach((group, idx) => {
                    ...
                    if (isStack && group.members.length > 0) {
                        stackCommon = {
                            group_id: group.main_row.group_id || null,   // ← добавить эту строку
            ...
            // Контекстное меню на строках оборудования
...
items.push({ text: 'Переместить', action: 'move', icon: 'assets/icons/move.png' });
            ...
            // Контекстное меню на строках детализации стека
...
function handleContextAction(action) {
    if (selectedNodeId) {
        switch (action) {
            ...
            case 'add_stack':
    console.log('add_stack triggered, selectedNodeId:', selectedNodeId);
    ...
    break;
            ...
            case 'show_rack':
    if (typeof openRackPanel === 'function') {
        openRackPanel(selectedEquipmentId);
    } else {
        showToast('Функция просмотра стойки недоступна', 'warning');
    }
    break;
        }
    }
...
async function deleteNode(nodeId) {
    ...
    if (result.success) removeNodeRow(nodeId);
    else showToast('Ошибка удаления: ' + result.error, 'error');
}
...
function showBuildingContextMenu(e, buildingId) { /* без изменений */ }
function addBuildingModal() { /* без изменений */ }
async function editBuilding(buildingId) { /* без изменений */ }
async function deleteBuilding(buildingId) { /* без изменений */ }
...
async function openMoveDialog(equipId, data) {
    ...
    if (modalTitle) {
        ...
        if (currentMoveEquipData && (!currentMoveEquipData.hostname || currentMoveEquipData.hostname.trim() === '')) {
            currentMoveEquipData.hostname = hostname;
        }
        if (data.Groupe == 2) {
            modalTitle.textContent = `Переместить стек ${hostname}`;
        } else {
            modalTitle.textContent = `Переместить устройство ${hostname}`;
        }
    }
...
function resetMoveModal() {
    ...
    safeStyle('move-stack-list', 'display', 'none');
    safeStyle('move-warehouse-select', 'display', 'none');
    safeStyle('move-submit-btn', 'display', 'none');
...
function submitMove() {
    ...
    if (stackOpts && stackOpts.style.display === 'block') {
        ...
        if (stackSelect && stackSelect.value) {
            ...
            // Выбрано "Как одиночное"
            formData.append('skip_stack_check', '1');
        }
    }
    ...
    fetch('?ajax=move_equipment', { method: 'POST', body: formData })
        ...
        .then(data => {
            if (data.need_stack_selection) {
                ...
                return; // не закрываем модальное окно, ждём повторного нажатия
            }
            if (data.success) {
                closeMoveModal();
...
items.push({ text: 'Переместить', action: 'move', icon: 'assets/icons/move.png' });
            items.push({ text: 'Настройка оповещения', action: 'alerts' });
            items.push({ text: 'Добавить в чек-лист', action: 'checklist', icon: 'assets/icons/checklist.png' });
            items.push({ text: 'Удалить', action: 'delete' });
            items.push({ text: 'Подробнее', action: 'detailed' });
    ...
    container.querySelectorAll('.equipment-stack-row').forEach(row => {
        row.addEventListener('click', function(e) {
            if (e.ctrlKey || e.metaKey) return;
            const equipId = this.dataset.equipmentId;
            if (equipId && typeof handleEquipmentClickForCabinet === 'function') {
                handleEquipmentClickForCabinet(equipId);
            }
        });
        row.addEventListener('contextmenu', function(e) {
            e.preventDefault();
...
async function loadNodeEquipmentContent(nodeId, container) {
    container.innerHTML = '<div class="no-equipment"><span class="load-indicator loading"></span> Загрузка...</div>';
    try {
        ...
        const visibleDetailColumns = detailColumns.filter(col => col !== 'id');
        const visibleDetailTitles = detailTitles.filter((_, i) => detailColumns[i] !== 'id');
        ...
        if (groups.length === 0) {
            ...
            groups.forEach((group, idx) => {
                ...
                if (isStack) {
                    html += `<tr class="equipment-detail-row" id="stack-detail-${gid}" style="display:none;"><td colspan="${visibleColumns.length + 2}">`;
                    html += '<table><thead><tr>';
                    visibleDetailTitles.forEach(title => html += `<th>${title}</th>`);
                    html += '<th><span class="add-col-btn" onclick="openAddColumnForm(\'equipment\')">+</span></th>';
                    html += '</tr></thead><tbody>';
                    group.members.forEach(member => {
                        html += `<tr data-equipment-id="${member.id}" class="equipment-stack-row">`;
                        visibleDetailColumns.forEach(col => {
                            let val = member[col] !== undefined ? member[col] : '';
                            if (col === 'status') {
                                const status = member[col] || 'inactive';
                                const dotClass = status === 'active' ? 'active' : (status === 'partial' ? 'partial' : 'inactive');
...
// ========== ФОРМА СТЕКА ==========
function openStackAddForm(nodeId, stackCommon) {
    const modal = document.getElementById('addStackDeviceModal');
    const form = document.getElementById('addStackDeviceForm');
    if (!modal || !form) return;

    // Заполняем скрытые поля (используем правильные ID из модалки)
    document.getElementById('stack-device-node').value = nodeId;
    document.getElementById('stack-device-ip').value = stackCommon.ip_address || '';
    document.getElementById('stack-device-hostname').value = stackCommon.hostname || '';
    document.getElementById('stack-device-type').value = stackCommon.device_type_id || '';
    document.getElementById('stack-device-vendor').value = stackCommon.vendor_id || '';
    // Поле model_id не имеет скрытого аналога, но можно задать через select позже

    const title = modal.querySelector('.modal-title') || modal.querySelector('h2, h3');
    if (title) title.textContent = `Добавление устройства в стек (${stackCommon.hostname || 'стек'})`;

    // Сбрасываем форму и восстанавливаем скрытые поля
    form.reset();
    document.getElementById('stack-device-node').value = nodeId;
    document.getElementById('stack-device-ip').value = stackCommon.ip_address || '';
    document.getElementById('stack-device-hostname').value = stackCommon.hostname || '';
    ...
    // Загружаем опции для селектов (прошивка, шкаф)
    loadStackSelects();
    modal.classList.add('visible');

    // Дополнительные обработчики (если есть)
    if (typeof setupFormSelectors === 'function') {
        setupFormSelectors(form);
    }
...
function closeStackForm() { const m = document.getElementById('addStackDeviceModal'); if (m) m.classList.remove('visible'); }

async function loadStackSelects() {
    const firmwareSelect = document.getElementById('stack-firmware-select');
    if (firmwareSelect) {
        if (firmwareSelect.searchableInstance) {
            firmwareSelect.searchableInstance.destroy();
            firmwareSelect.searchableInstance = null;
        }
        const wrapper = firmwareSelect.closest('.searchable-select');
        if (wrapper) {
            wrapper.parentNode.insertBefore(firmwareSelect, wrapper);
            wrapper.remove();
        }
        firmwareSelect.style.display = '';
        firmwareSelect.innerHTML = '<option value="">-- не выбрано --</option>';
        try {
            const fwList = await fetchJSON('?ajax=get_list&list=firmwares');
            const firmwares = fwList.data || [];
            firmwares.forEach(fw => firmwareSelect.appendChild(new Option(fw.name, fw.id)));
            new SearchableSelect(firmwareSelect);
        } catch (e) {}
    }

    const cabinetSelect = document.getElementById('stack-cabinet-select');
    if (cabinetSelect) {
        if (cabinetSelect.searchableInstance) {
            cabinetSelect.searchableInstance.destroy();
            cabinetSelect.searchableInstance = null;
        }
        const wrapper = cabinetSelect.closest('.searchable-select');
        if (wrapper) {
            wrapper.parentNode.insertBefore(cabinetSelect, wrapper);
            wrapper.remove();
        }
        cabinetSelect.style.display = '';
        cabinetSelect.innerHTML = '<option value="">-- не выбрано --</option>';
        try {
            const cabList = await fetchJSON('?ajax=get_list&list=cabinets');
            const cabinets = cabList.data || [];
            cabinets.forEach(cab => cabinetSelect.appendChild(new Option(cab.name, cab.id)));
            new SearchableSelect(cabinetSelect);
        } catch (e) {}
    }
}

// ---------- Обновление содержимого раскрытого узла без перезагрузки страницы ----------
...
function macBlurHandler(e) {
    const input = e.target;
    const mac = input.value.trim();
    if (mac.length === 14) {
        checkMacUnique(mac, input);
    } else {
        clearMacError(input);
    }
}

async function checkMacUnique(mac, inputElement) {
    try {
        const response = await fetch(`?ajax=check_mac_unique&mac=${encodeURIComponent(mac)}`);
        const data = await response.json();
        if (data.exists) {
            showMacError(inputElement, 'Устройство с таким MAC-адресом уже есть');
        } else {
            clearMacError(inputElement);
        }
    } catch (err) {
        console.error('Ошибка проверки MAC-адреса:', err);
    }
}

function showMacError(input, message) {
    input.classList.add('mac-error');
    let errorSpan = input.parentNode.querySelector('.mac-error-msg');
    if (!errorSpan) {
        errorSpan = document.createElement('span');
        errorSpan.className = 'mac-error-msg';
...
function clearMacError(input) {
    input.classList.remove('mac-error');
    const errorSpan = input.parentNode.querySelector('.mac-error-msg');
    if (errorSpan) errorSpan.remove();
}

// Инициализация обработчиков форм и MAC-адреса
// document.addEventListener('DOMContentLoaded', () => {
//     const stackForm = document.getElementById('addStackDeviceForm');
//     if (stackForm) {
//         stackForm.addEventListener('submit', function(e) {
//             e.preventDefault();
//             const formData = new FormData(this);
//             fetch('api/AddData/add_equipment.php', { method: 'POST', body: formData })
//                 .then(r => r.json())
//                 .then(data => {
//                     if (data.success) {
//                         closeStackForm();
//                         const nodeId = document.getElementById('stack-node-id').value;
//                         if (nodeId && typeof refreshNodeEquipment === 'function') {
//                             refreshNodeEquipment(nodeId);
//                         }
//                     } else {
//                         alert('Ошибка: ' + (data.error || 'неизвестная ошибка'));
...
//     if (alertForm) alertForm.addEventListener('submit', e => { e.preventDefault(); alert('Настройки сохранены (заглушка)'); closeAlertSettings(); });
//     const checklistForm = document.getElementById('checklistForm');
//     if (checklistForm) {
//         checklistForm.addEventListener('submit', function(e) {
//             e.preventDefault();
//             const formData = new FormData(this);
//             fetch('?ajax=add_checklist_item', { method: 'POST', body: formData })
//                 .then(r => r.json())
//                 .then(data => { if (data.success) { closeChecklistModal(); alert('Задача добавлена'); location.reload(); } else alert('Ошибка: ' + data.error); });
//         });
//     }

//     const macInputs = document.querySelectorAll('.mac-address');
...
async function searchNodes(query) {
    ...
    try {
        ...
        const result = await response.json();
        const nodes = result.nodes || result;          // поддержка старого формата
        ...
        if (nodes.length === 0) {
            if (warehouseMatch) {
                const display = warehouseMatch.hostname
                    || warehouseMatch.serial_number
                    || warehouseMatch.mac_address;
        ...
        // Узлы найдены – строим строки таблицы
        nodes.forEach(node => {
        ...
        // Если найден ещё и склад, покажем toast
        if (warehouseMatch) {
            const display = warehouseMatch.hostname
                || warehouseMatch.serial_number
                || warehouseMatch.mac_address;
...
async function editNode(nodeId) {
    try {
        ...
        alert('Ошибка загрузки данных узла');
    }
}
...
function openNodeAddForm() {
    ...
    openAddForm('node', null, null, { building_id: currentBuildingFilter || 0, building_name: name });
}
...
function showStackSelection(data) {
    ...
    window._stackSelectionData = data;
}
function cancelStackSelection() {
    const area = document.getElementById('stack-selection-area');
    area.style.display = 'none';
    // Восстанавливаем обычные элементы, но в контексте диалога перемещения проще закрыть всё
    closeMoveModal();
}

function submitMoveWithStack(groupId, slot) {
    const data = window._stackSelectionData;
    const formData = new FormData();
